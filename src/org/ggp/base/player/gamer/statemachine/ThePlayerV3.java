...
//  double[] heur_weight = new double[3];
  ...
  double[] heur_log_soc = {0.0, 0.0, 0.0};
  double[] heur_log_cou = {0.0, 0.0, 0.0};
  int heur = 0;
	...
	private double getHeuristicScore_ori(MachineState state) throws MoveDefinitionException, GoalDefinitionException{
	  double score = 0;
	  double normalize = 0;
    if (heur_weight[0] > 0){
	...
	public void stateMachineMetaGame(long timeout) throws TransitionDefinitionException, MoveDefinitionException, GoalDefinitionException
	{
		...
		if (getCurrentState() == stateMachine.getInitialState()) {
			Boolean check = false;
			for (int i = 0; i < 3; i++) {
				if (heur_log_cou[i] < 20) {
					heur = i;
					check = true;
				}
			}
			if (check == false) {
				...
				for (int i = 0; i < 3; i++) {
					if (heur_log_soc[i] / heur_log_cou[i] > maxV) {
						maxV = heur_log_soc[i] / heur_log_cou[i];
						heur = i;
					}
				}
			}
		...
		while(true) {
			...
			if (isTerminal) {
				heur_log_soc[heur] += stateMachine.getGoal(currentState, getRole());
			}
